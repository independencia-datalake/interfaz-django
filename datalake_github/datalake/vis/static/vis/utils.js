...
var mapStreets = (data) => {
  ...
  data.forEach( a => {
    let uv = a.UV;
    let conditions;
    if (a.UV.length < 3) uv = +a.UV;

    else {
      uv = uv.split('/').slice(1,uv.length)
      // remove outer spaces and start of string, split cond/result
      uv = uv.map( v => v.trim().substring(9,v.length).split(' -> '));
      uv = uv.map( v => [v[0], +v[1].substring(6,v[1].length)])
      ...
      uv.forEach( v => {
        ...
        let multi = false;
        if (/IMPAR/.test(c.str)) c.parity = 2;
        else if (/PAR/.test(c.str)) c.parity = 1;

        if (/> [0-9]*/.test(c.str)) c.range[0] = /> [0-9]*/.exec(c.str)[0].substring(2)*1+1;
        if (/< [0-9]*/.test(c.str)) c.range[1] = /< [0-9]*/.exec(c.str)[0].substring(2)*1-1;
        if (/>= [0-9]*/.test(c.str)) c.range[0] = />= [0-9]*/.exec(c.str)[0].substring(3)*1;
        if (/<= [0-9]*/.test(c.str)) c.range[1] = /<= [0-9]*/.exec(c.str)[0].substring(3)*1;
        else {
          // Este primer if no sirve para nada pero Eduado Frei tiene un caso
          if (/\= [0-9]*/.test(c.str)) {
            c.n = /[0-9]*/.exec(c.str)[0]*1;
            c.type = 'stand-alone';
          }
          if (/= \(.*\)/.test(c.str)) {
            let s = /= \(.*\)/.exec(c.str)[0].substring(3, c.str.length-1).trim().split(' ');
            s.forEach( b => conditions.push({
              'n': b*1,
              'type': 'stand-alone',
              'uv': v[1]
            }))
        ...
        c.type === 'complex'? c.n = c.range[0] || 0: null;
        if (!multi) conditions.push(c);
      })
    }
    if (conditions) conditions = conditions.sort( (a, b) => a.n > b.n? 1: -1);
    streets[a.STREET] = conditions || uv;
  })

  console.log(streets);

  let addressToUV = ( street, number ) => {

    try {
      let uv, conditions;
      conditions = streets[street];
      if (typeof conditions === 'number') uv = conditions;
      else {
        let c;
        for (let i = 0 ; i < conditions.length; i++) {
          let a = conditions[i];
          if (a.n == number && a.type === 'stand-alone') {
            uv = a.uv, c = conditions[i];
            break;
          }
          else if (a.n <= number && (!a.parity || (number%2)+1 == a.parity)) {
            uv = a.uv, c = conditions[i];
          }
          else if (a.n > number) break;
        }
      }
      return uv;
    } catch (e) {
